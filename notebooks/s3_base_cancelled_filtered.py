...
TABELAS = [
    ...
    "insurance_status", "representante", "catalogo" # Certifique-se que todas as tabelas do SQL estejam aqui
]
...
SELECT_TEMPLATE = """
    ...
    LEFT JOIN {schema}.representante r ON r.codigo = irs.id_unity
    LEFT JOIN {schema}.catalogo cata ON cata.cnpj_cpf = r.cnpj_cpf
"""
...
for today in daterange(DATA_INICIO, DATA_FIM):
    ...
    try:
        ...
        # 2. Identificadores (apenas se não estiverem vazios)
        if not df_integrais.empty:
        ...
        # Se não houver dados, define 0 e continua
...
'TRANSDESK DIGITAL LTDA'
        ...
        # Filtrar apenas pelas unidades especificadas
        if 'unidade' in df_cancelamentos.columns:
            df_cancelamentos = df_cancelamentos[df_cancelamentos['unidade'].isin(unidades_permitidas)].copy()
        else:
