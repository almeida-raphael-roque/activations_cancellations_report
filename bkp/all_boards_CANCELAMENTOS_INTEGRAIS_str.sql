...
SELECT
    -- DADOS DO VEÍCULO / REBOQUE
        ...
        TRY_CAST(coalesce(iv.id,it.id,itt.id) AS BIGINT) as "id_placa",
        TRY_CAST(coalesce(irsc.ID_VEHICLE) AS BIGINT) as "id_veiculo",
        TRY_CAST(coalesce(irsct.ID_TRAILER) AS BIGINT) as "id_carroceria",
    -- DADOS DO CONTRATO
        TRY_CAST(ir.id AS BIGINT) as "matricula",
        TRY_CAST(irs.id AS BIGINT) as "conjunto",
    ...
    -- DADOS DO PRODUTO
        TRY_CAST(irsc.id AS BIGINT) as "coverage_id",
        b.description as "beneficio",
        isss.description as "status_beneficio",
    -- DADOS DE DATAS
        current_date AS "data_extracao",
        TRY_CAST(irs.data_registration AS TIMESTAMP) as "data_registro",
        TRY_CAST(coalesce(TRY_CAST(irs.DATE_INITAL_EFFECT AS DATE), date_add('day', -364, TRY_CAST(irs.DATE_FINAL_EFFECT AS DATE))) AS DATE) as "data_ativacao",
        TRY_CAST(coalesce(TRY_CAST(irsc.date_initial_effect AS DATE), date_add('day', -364, TRY_CAST(irsc.date_final_effect AS DATE))) AS DATE) as "data_ativacao_beneficio",
        TRY_CAST(irs.date_cancellation AS DATE) as "data_cancelamento",
    -- DADOS DE ORDENAMENTO
        ROW_NUMBER() OVER (PARTITION BY coalesce(iv.chassi,it.chassi,itt.chassi) ORDER BY TRY_CAST(irs.data_registration AS TIMESTAMP) DESC) as "rn",
    -- DADOS DA EMPRESA
        'Segtruck' as empresa

    FROM silver.insurance_registration ir
        LEFT JOIN silver.insurance_reg_set irs ON TRY_CAST(irs.parent AS BIGINT) = ir.id
        LEFT JOIN silver.cliente clie ON clie.codigo = ir.CUSTOMER_ID
        LEFT JOIN silver.catalogo cat ON cat.cnpj_cpf = clie.cnpj_cpf
        LEFT JOIN silver.representante r ON r.codigo = TRY_CAST(irs.id_unity AS BIGINT)
        LEFT JOIN silver.catalogo cata ON cata.cnpj_cpf = r.cnpj_cpf
        LEFT JOIN silver.insurance_status iss ON iss.id = TRY_CAST(irs.id_status AS BIGINT)
        LEFT JOIN silver.INSURANCE_REG_SET_COVERAGE irsc ON irsc.PARENT = TRY_CAST(irs.ID AS BIGINT)
        LEFT JOIN silver.INSURANCE_VEHICLE iv ON TRY_CAST(iv.ID AS BIGINT) = irsc.ID_VEHICLE
        LEFT JOIN silver.TIPO_VEICULO tv ON tv.CODIGO = TRY_CAST(iv.CODE_TYPE_VEHICLE AS BIGINT) 
        LEFT JOIN silver.INSURANCE_REG_SET_COV_TRAILER irsct ON irsct.PARENT = irsc.ID
        LEFT JOIN silver.INSURANCE_TRAILER it ON TRY_CAST(it.ID AS BIGINT) = irsct.ID_TRAILER
        LEFT JOIN silver.insurance_trailer itt ON TRY_CAST(itt.id AS BIGINT) = irsc.ID_TRAILER
        LEFT JOIN silver.insurance_reg_historic irh ON TRY_CAST(irh.id_set AS BIGINT) = TRY_CAST(irs.id AS BIGINT)
        LEFT JOIN silver.vendedor v ON TRY_CAST(v.CODIGO AS BIGINT) = TRY_CAST(irs.ID_CONSULTANT AS BIGINT)
        ...
        LEFT JOIN (
            ...
            INNER JOIN silver.web_user wb ON TRY_CAST(wb.id AS BIGINT) = TRY_CAST(irs.id_user_support AS BIGINT)
        ) as user_name ON TRY_CAST(user_name.conjunto AS BIGINT) = TRY_CAST(irs.id AS BIGINT)
