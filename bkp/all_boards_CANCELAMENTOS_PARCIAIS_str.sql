...
SELECT DISTINCT
    ...
    -- DADOS DE DATAS
        ...
        + (TRY_CAST(irsc.UPDATED_AT AS BIGINT) - 599266080000000000) / 864000000000 * INTERVAL '1' DAY AS "data_atualizacao",
    -- DADOS DA EMPRESA
        'Segtruck' as empresa

    from silver.insurance_registration ir
        left outer join silver.insurance_reg_set irs on TRY_CAST(irs.parent AS BIGINT) = ir.id
        ...
        left outer join silver.representante r on r.codigo = TRY_CAST(irs.id_unity AS BIGINT)
        left outer join silver.catalogo cata on cata.cnpj_cpf = r.cnpj_cpf
        left outer join silver.insurance_status iss on iss.id = TRY_CAST(irs.id_status AS BIGINT)
        left outer join silver.INSURANCE_REG_SET_COVERAGE irsc ON irsc.PARENT = TRY_CAST(irs.ID AS BIGINT)
        left outer join silver.INSURANCE_VEHICLE iv on TRY_CAST(iv.ID AS BIGINT) = irsc.ID_VEHICLE
        left outer join silver.TIPO_VEICULO tv on tv.CODIGO = TRY_CAST(iv.CODE_TYPE_VEHICLE AS BIGINT) 
        left outer join silver.INSURANCE_REG_SET_COV_TRAILER irsct on irsct.PARENT = irsc.ID
        left outer join silver.INSURANCE_TRAILER it on TRY_CAST(it.ID AS BIGINT) = irsct.ID_TRAILER
        left outer join silver.insurance_trailer itt on TRY_CAST(itt.id AS BIGINT) = irsc.ID_TRAILER
        left outer join silver.vendedor v on TRY_CAST(v.CODIGO AS BIGINT) = TRY_CAST(irs.ID_CONSULTANT AS BIGINT)
    ...
    and CAST('1900-01-01 00:00:00' AS TIMESTAMP) 
        + (TRY_CAST(irsc.UPDATED_AT AS BIGINT) - 599266080000000000) / 864000000000 * INTERVAL '1' DAY >= TIMESTAMP '2025-01-01 00:00:00'
